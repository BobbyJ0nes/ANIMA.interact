...
export const dream = { visited: {}, pass: false, jacket: 'none' };
...
function buildNotation(scene) {
  ...
  const extra = n.extra || [];
  CHARS = [...new Set([' ', ...CORE_ALWAYS, ...RAMP, ...WRITING, ...extra])];
...
export function transition(nextId, spawn, opts = {}) {
  ...
  trans = {
    ...
    dur: opts.dur ?? 1.1, style: opts.style || 'veil',
  };
}
...
function renderMargin(t) {
  ...
  if (!marginCur) return;
  const m = marginCur;
  const shown = Math.floor((t - m.t0) * 26);
  const done = shown >= m.text.length;
  const hold = 1.7 + m.text.length * 0.02;
  const age = done ? t - m.t0 - m.text.length / 26 : 0;
...
function renderSky(t) {
  ...
  // the depletion arc — every scene visited, a few more stars have moved
  // out; the boat's emptied sky is where the arc was always going
  const visitedN = Object.keys(dream.visited || {}).length;
  const depl = Math.max(0.15, 1 - 0.09 * Math.max(0, visitedN - 1));
...
function renderTerrain(t) {
  ...
  const horizon = ROWS * 0.42 + cam.pitch * ROWS * 0.9;
  PROJ = ROWS * 1.05;
  const FOG_K = STYLE.fogK * (0.85 + env.ch.depth * 0.35);
  const LL = STYLE.letterLand;
  ...
  const rampCeil = Math.min(STYLE.rampMax, rampLen - 1);
...
function veilOverlay(t) {
  if (!trans) return;
  const tt = t - trans.t0;
  const f = trans.phase === 'out' ? Math.min(1, tt / trans.dur) : 1 - Math.min(1, tt / trans.dur);
  if (f <= 0.01) return;
  const cover = Math.pow(f, 1.15);
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      let hx, hz;
      if (trans.style === 'streak') { hx = c - Math.floor(t * 60); hz = r; }
      else if (trans.style === 'fall') { hx = c; hz = r - Math.floor(t * 45); }
      else { hx = c; hz = r + Math.floor(t * 8); }
      if (h2i(hx * 3 + 1, hz * 7 + 2) > cover) continue;
      const idx = r * COLS + c;
      const li = Math.floor(h2i(hx, hz + 13) * writIdx.length);
      charBuf[idx] = writIdx[li];
      slotBuf[idx] = 9 * N_STEPS + 2 + Math.floor(h2i(hx + 5, hz) * 3);
    }
  }
}
...
function frameLoop(now) {
  ...
  speechGlow *= 0.9;
  updateEnv(t);
  // the transition state machine — out, switch, in
  if (trans) {
    const tt = t - trans.t0;
...
export function initEngine(scenes, nightDirector = null) {
  ...
  if (params.has('fresh')) {
    ...
    Object.assign(dream, { visited: {}, pass: false, jacket: 'none', folds: 0, stage: 0 });
