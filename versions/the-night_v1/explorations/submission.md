...
3. **hallucination-as-dream-fabric** (the horizon, argued not built).
   ...
   Eros fine-tune strand ("a stylistic membrane with nobody inside" is a
   *first-class critical finding*, not a failure), the ascii-voice seed
   (fate's line typesetting by proximity). frame as designed-and-argued
   next stage; the report gains a "limits and next" section from it.
